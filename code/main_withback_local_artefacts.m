...
for id_exp = 1:length(data_folder)
    ...
    try
        ...
        if downsample_flag
            local_artefacts_SNR_save_name = [large_artefacts_save_path 'withback_local_artefacts_SNR.mat'];
            local_artefacts_NoRM_save_name = [large_artefacts_save_path 'withback_local_artefacts_NoRM.mat'];
        else
            local_artefacts_SNR_save_name = [large_artefacts_save_path 'withback_local_artefacts_SNR.mat'];
            local_artefacts_NoRM_save_name = [large_artefacts_save_path 'withback_local_artefacts_NoRM_nodown.mat'];
        end
        ...
        if exist(local_artefacts_SNR_save_name)
            ...
            if data_ref_box(id_exp)==1
                if exist(local_artefacts_NoRM_save_name)
                    ...
                    [time_down, movie_down, rois_f0_down, rois_f_NoRM, neuropil0_down, neuropil_NoRM,...
                        snr0_down, snr_NoRM,shift_x_NoRM,shift_y_NoRM] = reassign_px_NoRM(data.movie_doc.movie_ruido,...
                    ...
                    rois_f_raw_down = rois_f_raw_avg(:,1:down_fact:end);
                    data_NoRM = sls_deconvolution(movie_down,data,rois_f_NoRM,rois_f_raw_down,framePeriod_NoRM);
                    rois_ca_NoRM = data_NoRM.C_df;
                    clear data_NoRM;
                    
                    snr_ca_NoRM = zeros(size(rois_ca_raw,1),1);
            ...
            tic;
            [rois_f0, rois_f_SNR, neuropil0, neuropil_SNR, snr0, snr_SNR,shift_x_roi,shift_y_roi] = reassign_px_snr(data.movie_doc.movie_ruido', data.A, data.ring, data.surrounding, data.freehand_scan, data.framePeriod);
            framePeriod_SNR = data.framePeriod;
            time_temp_SNR = toc;
            data_SNR = sls_deconvolution(data.movie_doc.movie_ruido',data,rois_f_SNR,rois_f_raw,framePeriod_SNR);
            rois_ca_SNR = data_SNR.C_df;
            clear data_SNR;
            snr_ca_SNR = zeros(size(rois_ca_raw,1),1);
            for id_roi = 1:size(rois_ca_raw,1)
                snr_ca_SNR(id_roi) = 10^(snr(rois_ca_SNR(id_roi,:),rois_f_SNR(id_roi,:)-rois_ca_SNR(id_roi,:))/10);
            end
            save(local_artefacts_SNR_save_name,'rois_f_raw','neuropil_raw',...
                'snr_raw','rois_ca_raw','snr_ca_raw','rois_f_SNR', 'neuropil_SNR',...
...
%             figure;
%             subplot(3,1,1); imagesc(data.frameTimes,[],rois_f_raw); colorbar; title('no reassigment'); caxis([0, max([rois_f_raw(:); rois_f_SNR(:); rois_f_NoRM(:)])])
%             subplot(3,1,2); imagesc(data.frameTimes,[],rois_f_SNR); colorbar; title('SNR-based reassigment'); caxis([0, max([rois_f_raw(:); rois_f_SNR(:); rois_f_NoRM(:)])])
%             subplot(3,1,3); imagesc(time_down,[],rois_f_NoRM); colorbar; title('NoRMCorr-based reassigment'); caxis([0, max([rois_f_raw(:); rois_f_SNR(:); rois_f_NoRM(:)])])
%             
%             figure;
%             subplot(2,2,[1,2]); plot([snr_raw(:) snr_SNR(:) snr_NoRM(:)],'-o'); xlabel('ROI'); ylabel('SNR');
%             legend('no reassignment','SNR-based px reassignment','NoRMCorr-based reassignment');
%             subplot(2,2,3);
%             scatter(snr_raw(:), snr_SNR(:));
%             hold on; plot(0:1:ceil(max([snr_raw(:); snr_SNR(:)])),0:1:ceil(max([snr_raw(:); snr_SNR(:)])),'--k');
%             xlabel('no reassignment'); ylabel('SNR-based px reassignment');
%             subplot(2,2,4);
%             scatter(snr_raw(:), snr_NoRM(:));
%             hold on; plot(0:1:ceil(max([snr_raw(:); snr_NoRM(:)])),0:1:ceil(max([snr_raw(:); snr_NoRM(:)])),'--k');
%             xlabel('no reassignment'); ylabel('NoRMCorr-based px reassignment');
%             
%             figure;
%             subplot(3,1,1); imagesc(data.frameTimes,[],rois_ca_raw); colorbar; title('no reassigment'); caxis([0, max([rois_ca_raw(:); rois_ca_SNR(:); rois_ca_NoRM(:)])])
%             subplot(3,1,2); imagesc(data.frameTimes,[],rois_ca_SNR); colorbar; title('SNR-based reassigment'); caxis([0, max([rois_ca_raw(:); rois_ca_SNR(:); rois_ca_NoRM(:)])])
%             subplot(3,1,3); imagesc(time_down,[],rois_ca_NoRM); colorbar; title('NoRMCorr-based reassigment'); caxis([0, max([rois_ca_raw(:); rois_ca_SNR(:); rois_ca_NoRM(:)])])
%             
%             figure;
%             subplot(2,2,[1,2]); plot([snr_ca_raw(:) snr_ca_SNR(:) snr_ca_NoRM(:)],'-o'); xlabel('ROI'); ylabel('SNR deconvolved');
%             legend('no reassignment','SNR-based px reassignment','NoRMCorr-based reassignment');
%             subplot(2,2,3);
%             scatter(snr_ca_raw(:), snr_ca_SNR(:));
%             hold on; plot(0:1e-12:max([snr_ca_raw(:); snr_ca_SNR(:)]),0:1e-12:max([snr_ca_raw(:); snr_ca_SNR(:)]),'--k');
%             xlabel('no reassignment'); ylabel('SNR-based px reassignment');
%             subplot(2,2,4);
%             scatter(snr_ca_raw(:), snr_ca_NoRM(:));
%             hold on; plot(0:1e-12:max([snr_ca_raw(:); snr_ca_NoRM(:)]),0:1e-12:max([snr_ca_raw(:); snr_ca_NoRM(:)]),'--k');
%             xlabel('no reassignment'); ylabel('NoRMCorr-based px reassignment');
        end
        time_SNR(id_exp) = time_temp_SNR;
        surr_pooled = [surr_pooled; data_surround(id_exp)*ones(length(snr_raw),1)];
        
        snr_f_avg_raw(id_exp) = nanmean(snr_raw);
        snr_f_avg_SNR(id_exp) = nanmean(snr_SNR);

        snr_f_pooled_raw = [snr_f_pooled_raw; snr_raw];
        snr_f_pooled_SNR = [snr_f_pooled_SNR; snr_SNR];

        snr_ca_avg_raw(id_exp) = nanmean(snr_ca_raw);
        snr_ca_avg_SNR(id_exp) = nanmean(snr_ca_SNR);

        snr_ca_pooled_raw = [snr_ca_pooled_raw; snr_ca_raw];
...
%% save results
local_artefacts_table = array2table([time_SNR(:) time_NoRM(:) snr_f_avg_raw(:)...
    snr_f_avg_SNR(:) snr_f_avg_NoRM(:) snr_ca_avg_raw(:)...
    snr_ca_avg_SNR(:) snr_ca_avg_NoRM(:)],...
    'VariableNames',{'time_SNR','time_NoRM','snr_f_avg_raw','snr_f_avg_SNR',...
    'snr_f_avg_NoRM','snr_ca_avg_raw','snr_ca_avg_SNR','snr_ca_avg_NoRM'});
...
local_artefacts_pooled_table = array2table([surr_pooled(:) surr_pooled_NoRM_temp(:) snr_f_pooled_raw(:) snr_f_pooled_SNR(:)...
    snr_f_pooled_NoRM_temp(:) snr_ca_pooled_raw(:) snr_ca_pooled_SNR(:) snr_ca_pooled_NoRM_temp(:)],...
    'VariableNames',{'surr_pooled', 'surr_pooled_NoRM_temp', 'snr_f_pooled_raw','snr_f_pooled_SNR',...
    'snr_f_pooled_NoRM_temp','snr_ca_pooled_raw','snr_ca_pooled_SNR','snr_ca_pooled_NoRM_temp'});
if anesthetized == 1
    if downsample_flag
        writetable(local_artefacts_table,[save_path_0 'anesthetized_withback_local_artefacts_summary.csv']);
        writetable(local_artefacts_pooled_table,[save_path_0 'anesthetized_withback_local_artefacts_pooled.csv']);
    else
        writetable(local_artefacts_table,[save_path_0 'anesthetized_withback_local_artefacts_summary_nodown.csv']);
        writetable(local_artefacts_pooled_table,[save_path_0 'anesthetized_withback_local_artefacts_pooled_nodown.csv']);
    end
else
    if downsample_flag
        writetable(local_artefacts_table,[save_path_0 'awake_withback_local_artefacts_summary.csv']);
        writetable(local_artefacts_pooled_table,[save_path_0 'awake_withback_local_artefacts_pooled.csv']);
    else
        writetable(local_artefacts_table,[save_path_0 'awake_withback_local_artefacts_summary_nodown.csv']);
        writetable(local_artefacts_pooled_table,[save_path_0 'awake_withback_local_artefacts_pooled_nodown.csv']);
    end
end
...
for i_s = 1:length(surr_array)
    snr_f_raw_temp = snr_f_pooled_raw(find(surr_pooled==surr_array(i_s)));
    ...
    b_w = 10;
    subplot(2,3,i_s);
    hold on; histogram(snr_f_raw_temp,'binWidth',b_w);
...
surr_array = unique(surr_pooled);
for i_s = 1:length(surr_array)
    snr_ca_raw_temp = snr_ca_pooled_raw(find(surr_pooled==surr_array(i_s)));
    ...
    b_w = 1e-7;
    subplot(2,3,i_s);
    hold on; histogram(snr_ca_raw_temp,'binWidth',b_w);
...
b_w = 50;
figure; histogram(snr_f_pooled_raw,'binWidth',b_w);
hold on; histogram(snr_f_pooled_SNR,'binWidth',b_w);
hold on; histogram(snr_f_pooled_NoRM,'binWidth',b_w);
legend('NO local process','SNR','NoRMCorr');
xlabel('average SNR fluo'); ylabel('num acquisitions');
disp(['average SNR without local processing=' num2str(nanmean(snr_f_pooled_raw)) '+/-' num2str(nanstd(snr_f_pooled_raw)/sqrt(sum(1-isnan(snr_f_pooled_raw)))) '(mean+/-sem)']);
...
b_w = 5e-8;
figure; histogram(snr_ca_pooled_raw,'binWidth',b_w);
...
disp(['average SNR without local processing=' num2str(nanmean(snr_ca_pooled_raw)) '+/-' num2str(nanstd(snr_ca_pooled_raw)/sqrt(sum(1-isnan(snr_ca_pooled_raw)))) '(mean+/-sem)']);
...
%     snr_ca_NoRM_temp = snr_ca_avg_NoRM(find(data_surround==data_surround(i_s)));
    subplot(2,5,i_s);
    hold on; scatter(snr_f_raw_temp, snr_f_SNR_temp);
    hold on; plot(nanmin([snr_f_raw_temp; snr_f_SNR_temp]):1:nanmax([snr_f_raw_temp; snr_f_SNR_temp]),...
        nanmin([snr_f_raw_temp; snr_f_SNR_temp]):1:nanmax([snr_f_raw_temp; snr_f_SNR_temp]),'--k');
    xlabel('SNR fluo without local processing'); ylabel('SNR fluo after SNR-based correction');
    title(['surround=' num2str(surr_array(i_s))]);
    subplot(2,5,5+i_s);
    hold on; scatter(snr_f_raw_temp, snr_f_NoRM_temp);
    hold on; plot(nanmin([snr_f_raw_temp; snr_f_NoRM_temp]):1:nanmax([snr_f_raw_temp; snr_f_NoRM_temp]),...
        nanmin([snr_f_raw_temp; snr_f_NoRM_temp]):1:nanmax([snr_f_raw_temp; snr_f_NoRM_temp]),'--k');
...
surr_array = unique(data_surround);
for i_s = 1:length(surr_array)
    snr_ca_raw_temp = snr_ca_avg_raw(find(data_surround==surr_array(i_s)));
    ...
    subplot(2,5,i_s);
    hold on; scatter(snr_ca_raw_temp, snr_ca_SNR_temp);
    hold on; plot(nanmin([snr_ca_raw_temp; snr_ca_SNR_temp]):1e-10:nanmax([snr_ca_raw_temp; snr_ca_SNR_temp]),...
        nanmin([snr_ca_raw_temp; snr_ca_SNR_temp]):1e-10:nanmax([snr_ca_raw_temp; snr_ca_SNR_temp]),'--k');
    xlabel('SNR deconv without local processing'); ylabel('SNR deconv after SNR-based correction');
    title(['surround=' num2str(surr_array(i_s))]);
    subplot(2,5,5+i_s);
    hold on; scatter(snr_ca_raw_temp, snr_ca_NoRM_temp);
    hold on; plot(nanmin([snr_ca_raw_temp; snr_ca_NoRM_temp]):1e-10:nanmax([snr_ca_raw_temp; snr_ca_NoRM_temp]),...
        nanmin([snr_ca_raw_temp; snr_ca_NoRM_temp]):1e-10:nanmax([snr_ca_raw_temp; snr_ca_NoRM_temp]),'--k');
    xlabel('SNR deconv without local processing'); ylabel('SNR deconv after NoRMCorr-based correction');
    title(['surround=' num2str(surr_array(i_s))]);
end

figure; scatter(snr_f_avg_raw, snr_f_avg_SNR);
hold on; plot(-1:1:1,-1:1:1,'--k');
xlabel('SNR fluo without local processing'); ylabel('SNR fluo after SNR-based correction');
disp(['average SNR without local processing=' num2str(nanmean(snr_f_avg_raw)) '+/-' num2str(nanstd(snr_f_avg_raw)/sqrt(sum(1-isnan(snr_f_avg_raw)))) '(mean+/-sem)']);
...
figure; scatter(snr_f_avg_raw, snr_f_avg_NoRM);
hold on; plot(-1:1:1,-1:1:1,'--k');
xlabel('SNR fluo without local processing'); ylabel('SNR fluo after NoRMCorr-based correction');
disp(['average SNR without local processing=' num2str(nanmean(snr_f_avg_raw)) '+/-' num2str(nanstd(snr_f_avg_raw)/sqrt(sum(1-isnan(snr_f_avg_raw)))) '(mean+/-sem)']);
...
figure; scatter(snr_ca_avg_raw, snr_ca_avg_SNR);
hold on; plot(-1:1:1,-1:1:1,'--k');
xlabel('SNR deconvolved without local processing'); ylabel('SNR deconvolved after SNR-based correction');
disp(['average SNR without local processing=' num2str(nanmean(snr_ca_avg_raw)) '+/-' num2str(nanstd(snr_ca_avg_raw)/sqrt(sum(1-isnan(snr_ca_avg_raw)))) '(mean+/-sem)']);
...
figure; scatter(snr_ca_avg_raw, snr_ca_avg_NoRM);
hold on; plot(-1:1:1,-1:1:1,'--k');
