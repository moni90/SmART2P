...
%% run analyses
err_data = string([]);
time_back_SNR = NaN * ones(length(id_keep_def),1);
time_back_NoRM = NaN * ones(length(id_keep_def),length(down_rate));
num_ROIs = NaN * ones(length(id_keep_def),length(down_rate));

snr_f_avg_raw = NaN * ones(length(id_keep_def),length(down_rate));
snr_f_avg_back_SNR = NaN * ones(length(id_keep_def),1);
snr_f_avg_back_NoRM = NaN * ones(length(id_keep_def),length(down_rate));

snr_ca_avg_raw = NaN * ones(length(id_keep_def),length(down_rate));
snr_ca_avg_back_SNR = NaN * ones(length(id_keep_def),1);
snr_ca_avg_back_NoRM = NaN * ones(length(id_keep_def),length(down_rate));
...
for id_exp = 1:length(id_keep_def)
    ...
    try
        ...
        local_artefacts_diff_rate_save_name = [large_artefacts_save_path 'withback_local_artefacts_NoRM_diff_rate_v2.mat'];
...
%                 process_opt.down_rate = 2;
                ...
                shift_x_NoRM = zeros(length(down_rate),size(data.movie_doc.movie_ruido,1));
                shift_y_NoRM = zeros(length(down_rate),size(data.movie_doc.movie_ruido,1));
                shift_x_fig = figure;
                shift_y_fig = figure;
                snr_NoRM = zeros(size(rois_ca_raw,1),length(down_rate));
                snr_ca_NoRM = zeros(size(rois_ca_raw,1),length(down_rate));
                time_temp_NoRM = zeros(1,length(down_rate));
                rois_f_NoRM = zeros(size(rois_ca_raw,1),size(rois_ca_raw,2),length(down_rate));
                rois_ca_NoRM = zeros(size(rois_ca_raw,1),size(rois_ca_raw,2),length(down_rate));
                neuropil_NoRM = zeros(size(rois_ca_raw,1),size(rois_ca_raw,2),length(down_rate));
                for i_rate = 1:length(down_rate)
                    tic;
                    [time_down, movie_down, rois_f0_down, rois_f_NoRM_temp, neuropil0_down, neuropil_NoRM_temp,...
                        snr0_down, snr_NoRM_temp,shift_x_NoRM(i_rate,:),shift_y_NoRM(i_rate,:)] = reassign_px_NoRM_keep_rate(data.movie_doc.movie_ruido,...
                    ...
                    data_NoRM = sls_deconvolution(movie_avg,data,rois_f_NoRM(:,:,i_rate),rois_f_raw_avg,data.framePeriod);
...
%                     snr_ca_NoRM = zeros(size(rois_ca_noback,1),1);
        ...
        time_back_SNR(id_exp) = time_temp_SNR;
...
%         snr_ca_avg_raw(id_exp) = nanmean(snr_ca_raw);
        ...
        if data_ref_box(id_keep_def(id_exp))==1
            time_back_NoRM(id_exp,:) = time_temp_NoRM;
            
            snr_f_avg_back_NoRM(id_exp,:) = nanmean(snr_NoRM,1);
            snr_ca_avg_back_NoRM(id_exp,:) = nanmean(snr_ca_NoRM,1);
        end
...
%%
figure;
boxplot([snr_f_avg_back_SNR snr_f_avg_back_NoRM],...
...
for i_s = 1:length(unique(surr_array))
    snr_f_avg_SNR_temp =  snr_f_avg_back_SNR(data_surround==surr_array(i_s));
    snr_f_avg_NoRM_temp =  snr_f_avg_back_NoRM(data_surround==surr_array(i_s),:);
...
% data_surround_temp(data_surround_temp<2)=0;
surr_array = unique(data_surround_temp);
% surr_array(surr_array>2)=3;
for i_s = 1:length(unique(surr_array))
    snr_f_avg_raw_temp =  snr_f_avg_raw(data_surround_temp==surr_array(i_s));
    snr_f_avg_SNR_temp =  snr_f_avg_back_SNR(data_surround_temp==surr_array(i_s));
    snr_f_avg_NoRM_temp =  snr_f_avg_back_NoRM(data_surround_temp==surr_array(i_s),:);
    figure(box_plot_corr);    
    subplot(2,2,i_s);
    hold on; boxplot([snr_f_avg_raw_temp snr_f_avg_SNR_temp snr_f_avg_NoRM_temp],...
        'Labels',{'raw','SNR','NoRM 0.1Hz','NoRM 1Hz',...
        'NoRM'},'PlotStyle','traditional',...
        'orientation','horizontal');
    ylabel('snr');
    title(['surround=' num2str(surr_array(i_s))]);
    [p,tbl,stats] = kruskalwallis([snr_f_avg_raw_temp(:) snr_f_avg_SNR_temp(:), snr_f_avg_NoRM_temp],...
    {'raw','SNR','NoRM 0.1Hz','NoRM 1Hz','NoRM'});
    figure;
    c = multcompare(stats);
    title(['SNR. surround=' num2str(surr_array(i_s))]);
end
...
figure;
subplot(1,2,1);
imagesc(nanmean(corr_x_artefacts,3)); caxis([-1 1]);
title('dx'); colorbar;
subplot(1,2,2);
imagesc(nanmean(corr_y_artefacts,3)); caxis([-1 1]);
title('dy'); colorbar;